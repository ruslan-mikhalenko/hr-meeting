...
class TelegramService
{
        ...
        try {
            ...
            if (isset($chatInfo['participants']) && is_array($chatInfo['participants'])) {
                foreach ($chatInfo['participants'] as $participant) {
                    ...
                    // Добавляем участника в массив $allParticipants с корректным ключом и значениями
                    $allParticipants[] = [
                        'telegram_user_id' => $user['id'] ?? null, // Используем telegram_user_id вместо user_id
                        'first_name' => $user['first_name'] ?? null,
                        'last_name' => $user['last_name'] ?? null,
                        'username' => $user['username'] ?? null,
                        'phone' => $user['phone'] ?? null,
                        'status' => $user['status'] ?? null,       // Статус активности (например, online/offline)
                        'role' => $participant['_'] ?? null,       // Роль участника в чате (например, admin/member)
                    ];
                }
            }
        } catch (\Exception $e) {
            // Логирование ошибки, если не удалось получить участников
    ...
    public function trackNewSubscribers(string $channelId)
    {
        try {
            ...
            $currentSubscribers = array_column($allParticipants, null, 'telegram_user_id'); // Индексируем участников по user_id

            // Получаем подписчиков из базы данных
            $storedSubscribers = DB::table('subscribers')
                ->select('telegram_user_id', 'is_active')
                ->where('project_id', $project->id) // Выборка только для текущего канала (проекта)
                ->get()
                ->keyBy('telegram_user_id') // Индексируем по telegram_user_id
                ->toArray();
            ...
            Log::info("Новые подписчики: " . json_encode($newSubscribers));
            Log::info("Вернувшиеся подписчики: " . json_encode($returningSubscribers));
