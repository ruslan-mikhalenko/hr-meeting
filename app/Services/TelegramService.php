...
class TelegramService
{
    ...
    //Теперь при получении новых участников ваш бот будет проверять их ID и исключать свои собственные действия. Это исключает зацикливание отправки сообщений.
    public function getBotId()
    {
        try {
            // Получаем информацию о текущем боте
            $me = $this->madeline->getSelf();
            return $me['id'] ?? null;
        } catch (\Exception $e) {
            logger()->error("Ошибка получения идентификатора бота: {$e->getMessage()}");
            return null;
        }
    }
