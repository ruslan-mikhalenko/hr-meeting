...
const submitAddForm = () => {
  ...
  axios
    ...
    .then(function (response) {
      ...
      localUsers.value.unshift(response.data.user); // Добавляем нового пользователя
      successMessage.value =
        "Клиент успешно добавлена! Данные доступа к личному кабинету отправлены ему на email.";
      ...
      if (
        ...
        pagination.value.total > (pagination.value.last_page - 1) * usersPerPage
      ) {
      ...
      if (localUsers.value.length === 0) {
        ...
        fetchUsers(pagination.value.current_page); // Загружаем пользователей с первой страницы
      }
    })
    .catch((error) => {
      if (error.response && error.response.data.errors) {
...
function openModalEdit(id) {
  ...
  axios
    ...
    .then(function (response) {
      ...
      form.unp = response.data.user.unp;
      editUserId.value = response.data.user.id; // Сохраняем id для обновления
...
const submitEditForm = () => {
  ...
  axios
    ...
    .then(function (response) {
      successMessage.value = "Клиент успешно обновлёна!";

      // Обновление существующего пользователя
      const updatedUser = response.data.user; // Это ваш обновленный объект
                ...
                <form @submit.prevent="submitEditForm" autocomplete="off">
                  ...
                  <div class="mb-4">
                    ...
                    <input
                      ...
                      :class="{ 'border-red-500': errors.name }"
                    />
                    <span v-if="errors.telega" class="text-red-500 err">{{
                      errors.telega[0]
                    }}</span>
                  </div>
